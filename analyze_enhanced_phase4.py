...
def create_training_plots(history):
    ...
    # Motor loss curves
    axes[0, 1].plot(epochs, history['train_motor_losses'], label='Train Motor', alpha=0.8)
    ...
    axes[0, 2].plot(epochs, history['train_cognitive_losses'], label='Train Cognitive', alpha=0.8)
    axes[0, 2].plot(epochs, history['val_cognitive_losses'], label='Val Cognitive', alpha=0.8)
    ...
    overfitting_ratio = [v/t if t > 0 else 1 for v, t in 
                        zip(history['val_losses'], history['train_losses'])]
    axes[1, 1].plot(epochs, overfitting_ratio, alpha=0.8, color='red')
    axes[1, 1].axhline(y=1.0, color='black', linestyle='--', alpha=0.5, label='Perfect Balance')
    axes[1, 1].axhline(y=1.5, color='orange', linestyle='--', alpha=0.5, label='Moderate Overfitting')
    axes[1, 1].axhline(y=3.0, color='red', linestyle='--', alpha=0.5, label='Severe Overfitting')
...
def main():
    ...
    try:
        ...
        logger.info("\n✅ Analysis complete!")
        logger.info("📊 Check 'enhanced_phase4_training_analysis.png' for visualizations")
