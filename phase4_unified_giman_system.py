...
class UnifiedGIMANSystem(nn.Module):
    ...
    def _compute_loss(self, motor_pred, motor_true, cognitive_pred, cognitive_true, attention_weights):
        """Computes the combined loss for the model."""
...
def run_phase4_experiment(
    ...
    # Data scaling and conversion to tensors
    scaler = StandardScaler()
    motor_targets_scaled = scaler.fit_transform(motor_targets.reshape(-1, 1)).flatten()

    X_spatial = torch.tensor(spatial_emb, dtype=torch.float32)
    X_genomic = torch.tensor(genomic_emb, dtype=torch.float32)
    X_temporal = torch.tensor(temporal_emb, dtype=torch.float32)
    y_motor = torch.tensor(motor_targets_scaled, dtype=torch.float32)
    y_cognitive = torch.tensor(cognitive_targets, dtype=torch.float32)

    # Create dataset and dataloaders
    dataset = TensorDataset(X_spatial, X_genomic, X_temporal, y_motor, y_cognitive)
    
    # Data Validation Step
    for i, tensor in enumerate([X_spatial, X_genomic, X_temporal, y_motor, y_cognitive]):
        if torch.isnan(tensor).any() or torch.isinf(tensor).any():
            logging.error(f"NaN or Inf found in tensor at index {i}. Aborting.")
            # Replace NaNs/Infs with zeros as a fallback
            tensor[torch.isnan(tensor) | torch.isinf(tensor)] = 0
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            loss, _, _ = model._compute_loss(motor_pred, y_m, cog_pred, y_c, att_weights)
            loss.backward()
    ...
    logging.info(f"Final Validation -> Motor R²: {motor_r2:.4f}, Cognitive AUC: {cognitive_auc:.4f}")
...
class GIMANResearchAnalyzer:
    ...
    def create_unified_embeddings(self):
        ...
        for patno in self.patient_ids:
            ...
            genomic_emb = self._create_genomic_embedding(patient_genetic[genetic_features].values)
        ...
        for emb in [self.spatiotemporal_embeddings, self.genomic_embeddings, self.temporal_embeddings]:
    ...
    def _create_spatial_embedding(self, imaging_data: np.ndarray, target_dim: int = 256) -> np.ndarray:
    ...
    def _create_genomic_embedding(self, genetic_data: np.ndarray, target_dim: int = 256) -> np.ndarray:
    ...
    def train_unified_system(self, num_epochs: int = 100) -> Dict:
        ...
        optimizer = torch.optim.AdamW(self.unified_model.parameters(), lr=5e-4, weight_decay=1e-3)
        ...
        for epoch in range(num_epochs):
            ...
            ensemble_reg = 0.01 * (
                torch.mean(train_outputs['motor_ensemble_info']['ensemble_weights'] ** 2) +
                torch.mean(train_outputs['cognitive_ensemble_info']['ensemble_weights'] ** 2)
            )
            
            train_loss = 1.5 * motor_loss + cognitive_loss + attention_reg + ensemble_reg
            train_loss.backward()
        ...
        logger.info(f"✅ Unified training completed!")
        logger.info(f"📈 Motor R²: {motor_r2:.4f}, Correlation: {motor_corr:.4f}")
        logger.info(f"🧠 Cognitive AUC: {cognitive_auc:.4f}, Accuracy: {cognitive_acc:.4f}")
...
def main():
    ...
    print(f"🧠 Unified architecture: Cross-modal + Temporal + Ensemble")
