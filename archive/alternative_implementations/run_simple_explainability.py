...
def load_trained_model():
    ...
    print(
        f"   📊 Graph data loaded: {graph_data.x.shape[0]} nodes, {graph_data.x.shape[1]} features"
    )
    ...
    print(
        f"   ✅ Model loaded successfully: {sum(p.numel() for p in model.parameters())} parameters"
    )
...
def analyze_node_importance(model, graph_data) -> dict[str, np.ndarray]:
    ...
    return {"node_importance": node_importance, "method": "gradient_magnitude"}
...
def analyze_feature_importance(model, graph_data) -> dict[str, np.ndarray]:
    ...
    if logits.shape[1] == 1:  # Binary classification
        ...
        feature_importance = (
            torch.abs(graph_data.x.grad).mean(dim=0).detach().cpu().numpy()
        )
...
def validate_model_authenticity(model, graph_data):
    ...
    print("\n🔍 VALIDATION SUMMARY:")
    print("   The model appears to be generating authentic predictions")
    print("   based on actual computation, not hardcoded values.")
    ...
    with torch.no_grad():
        ...
        if logits.shape[1] == 1:  # Binary classification
            ...
            for class_id in range(logits.shape[1]):
                ...
                print(
                    f"   • Class {class_id}: {count} nodes (avg prob: {avg_prob:.4f})"
                )
        ...
        if hasattr(graph_data, "y") and graph_data.y is not None:
            ...
            precision = precision_score(
                y_true, y_pred, average="binary" if logits.shape[1] == 1 else "macro"
            )
            recall = recall_score(
                y_true, y_pred, average="binary" if logits.shape[1] == 1 else "macro"
            )
...
def create_visualizations(node_results, feature_results, graph_data):
    ...
    axes[0, 0].hist(
        node_importance, bins=30, alpha=0.7, color="skyblue", edgecolor="black"
    )
    ...
    degrees = np.bincount(
        graph_data.edge_index[0].detach().cpu().numpy(), minlength=graph_data.x.shape[0]
    )
    ...
    im = axes[1, 2].imshow(
        feature_corr, cmap="coolwarm", aspect="auto", vmin=-1, vmax=1
    )
...
def main():
    ...
    try:
        ...
        print("🔍 KEY INSIGHTS:")
        print(
            f"   • Most important node: #{np.argmax(node_importance)} (score: {np.max(node_importance):.6f})"
        )
        print(
            f"   • Least important node: #{np.argmin(node_importance)} (score: {np.min(node_importance):.6f})"
        )
        print(
            f"   • Node importance range: {np.min(node_importance):.6f} - {np.max(node_importance):.6f}"
        )
        ...
        for i, feat_idx in enumerate(feature_results["importance_ranking"][:3]):
            print(
                f"   {i + 1}. {feature_names[feat_idx]}: {feature_importance[feat_idx]:.6f}"
            )

        degrees = np.bincount(
            graph_data.edge_index[0].detach().cpu().numpy(),
        ...
        print(
            f"   • Graph density: {2 * graph_data.edge_index.shape[1] / (graph_data.x.shape[0] * (graph_data.x.shape[0] - 1)):.4f}"
        )
