#!/usr/bin/env python3
"""GIMAN Phase 4: Regularization Calibration System
===============================================

Building on the architecturally sound ultra-regularized system, implementing
systematic regularization calibration to find the optimal balance between
overfitting prevention and learning capability.

Strategy:
- Experiment 1: Moderate Dropout (dropout 0.4, weight_decay 1e-3)
- Experiment 2: Moderate Weight Decay (dropout 0.6, weight_decay 1e-4)
- Experiment 3: Moderate Both (dropout 0.5, weight_decay 1e-4)
- Maintain 32-dim embeddings and node-level architecture
- Full LOOCV evaluation for each configuration
...
class CalibratedGAT(nn.Module):
    """GAT with calibrated regularization parameters."""
...
class CalibratedGIMAN(nn.Module):
    """GIMAN with calibrated regularization parameters."""
    ...
    def __init__(
        ...
        # GAT with calibrated dropout
        self.gat = CalibratedGAT(
        ...
        # Prediction heads with calibrated regularization
        ...
        # Initialize weights with calibrated variance
        self._initialize_weights()

    def _initialize_weights(self):
        """Initialize weights with moderate variance."""
...
class RegularizationCalibrator:
    """Systematic regularization calibration with LOOCV evaluation."""

    def __init__(self, device="cpu"):
        self.device = device
        self.experiments = [
            {
                "name": "Moderate Dropout",
                "description": "dropout=0.4, weight_decay=1e-3",
                "dropout_rate": 0.4,
                "weight_decay": 1e-3,
            },
            {
                "name": "Moderate Weight Decay",
                "description": "dropout=0.6, weight_decay=1e-4",
                "dropout_rate": 0.6,
                "weight_decay": 1e-4,
            },
            {
                "name": "Moderate Both",
                "description": "dropout=0.5, weight_decay=1e-4",
                "dropout_rate": 0.5,
                "weight_decay": 1e-4,
            },
        ]
    ...
    def run_calibration(
        ...
        """Run systematic regularization calibration."""
        results = {}

        logger.info("🎯 GIMAN REGULARIZATION CALIBRATION")
        logger.info("=" * 60)
    ...
    def _run_experiment(
        ...
        for fold, (train_idx, test_idx) in enumerate(loo.split(X_spatial)):
            ...
            model = CalibratedGIMAN(dropout_rate=dropout_rate).to(self.device)
    ...
    def _analyze_results(self, results):
        ...
        logger.info("=" * 60)
        logger.info("📊 CALIBRATION RESULTS ANALYSIS")
        logger.info("=" * 60)
        ...
        for name, result in results.items():
            ...
            if result["motor_r2"] > 0.0 and result["cognitive_auc"] > 0.6:
                ...
                logger.info("   ✅ GOOD: Positive R² achieved!")
            else:
        ...
        if best_combined_config[1]["motor_r2"] > 0.0:
            logger.info("✅ Optimal regularization found! Ready for next phase.")
            logger.info(f"   Recommended config: {best_combined_config[0]}")
        else:
            logger.info("⚠️  Further calibration needed. Consider:")
            logger.info("   - Additional intermediate regularization levels")
            logger.info("   - Ensemble methods")
            logger.info("   - Data augmentation techniques")
...
def main():
    ...
    logger.info("🎬 GIMAN Phase 4: Regularization Calibration")
    logger.info("=" * 60)
