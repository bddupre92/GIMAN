...
sys.path.append(
    "/Users/blair.dupre/Library/CloudStorage/GoogleDrive-dupre.blair92@gmail.com/My Drive/CSCI FALL 2025/archive/development/phase4"
)
from phase4.phase4_ultra_regularized_system import UltraRegularizedGIMANSystem, UltraRegularizedLOOCVEvaluator
...
class Phase4vs5Comparator:
    ...
    def __init__(self, device="cpu", results_dir: Optional[Path] = None):
    ...
    def run_comprehensive_comparison(self, X_spatial, X_genomic, X_temporal, y_motor, y_cognitive, adj_matrix):
        ...
        phase4_evaluator = UltraRegularizedLOOCVEvaluator(device=self.device)
    ...
    def evaluate_dynamic_loss_strategies(self, X_spatial, X_genomic, X_temporal, y_motor, y_cognitive, adj_matrix):
        ...
        for strategy in strategies:
            ...
            logger.info(f"   {strategy.upper()}: Motor R² = {results['motor_r2']:.4f}, Cognitive AUC = {results['cognitive_auc']:.4f}")
    ...
    def _compute_statistical_comparison(self):
        ...
        motor_improvement = self.phase5_results['motor_r2'] - self.phase4_results['motor_r2']
        motor_improvement_pct = (motor_improvement / abs(self.phase4_results['motor_r2'])) * 100 if self.phase4_results['motor_r2'] != 0 else 0
        
        # Cognitive classification comparison
        cognitive_improvement = self.phase5_results['cognitive_auc'] - self.phase4_results['cognitive_auc']
        cognitive_improvement_pct = (cognitive_improvement / self.phase4_results['cognitive_auc']) * 100
        ...
        logger.info(f"   Motor R² improvement: {motor_improvement:+.4f} ({motor_improvement_pct:+.1f}%)")
        logger.info(f"   Cognitive AUC improvement: {cognitive_improvement:+.4f} ({cognitive_improvement_pct:+.1f}%)")
    ...
    def _analyze_performance_improvements(self):
        ...
        phase4_balance = abs(self.phase4_results['motor_r2']) / (self.phase4_results['cognitive_auc'] + 1e-8)
        phase5_balance = abs(self.phase5_results['motor_r2']) / (self.phase5_results['cognitive_auc'] + 1e-8)
        balance_improvement = (phase5_balance - phase4_balance) / phase4_balance * 100
        
        # Overall performance score (weighted combination)
        phase4_score = 0.7 * self.phase4_results['motor_r2'] + 0.3 * self.phase4_results['cognitive_auc']
        phase5_score = 0.7 * self.phase5_results['motor_r2'] + 0.3 * self.phase5_results['cognitive_auc']
        overall_improvement = phase5_score - phase4_score
        
        # Best individual performance
        best_motor = max(self.phase4_results['motor_r2'], self.phase5_results['motor_r2'])
        best_cognitive = max(self.phase4_results['cognitive_auc'], self.phase5_results['cognitive_auc'])
    ...
    def _create_comparison_visualizations(self):
        ...
        fig.suptitle('Phase 4 vs Phase 5 GIMAN Comparison', fontsize=16, fontweight='bold')

        # Performance comparison bar chart
        ax1 = axes[0, 0]
        metrics = ['Motor R²', 'Cognitive AUC']
        phase4_values = [self.phase4_results['motor_r2'], self.phase4_results['cognitive_auc']]
        phase5_values = [self.phase5_results['motor_r2'], self.phase5_results['cognitive_auc']]
        
        x = np.arange(len(metrics))
        width = 0.35
        
        bars1 = ax1.bar(x - width/2, phase4_values, width, label='Phase 4', alpha=0.8, color='skyblue')
        bars2 = ax1.bar(x + width/2, phase5_values, width, label='Phase 5', alpha=0.8, color='lightcoral')
        ...
        ax2 = axes[0, 1]
        ax2.scatter(self.phase4_results['motor_actuals'], self.phase4_results['motor_predictions'], 
                   alpha=0.6, label='Phase 4', color='skyblue')
        ax2.scatter(self.phase5_results['motor_actuals'], self.phase5_results['motor_predictions'], 
                   alpha=0.6, label='Phase 5', color='lightcoral')
        
        # Perfect prediction line
        min_val = min(np.min(self.phase4_results['motor_actuals']), np.min(self.phase5_results['motor_actuals']))
        max_val = max(np.max(self.phase4_results['motor_actuals']), np.max(self.phase5_results['motor_actuals']))
        ...
        bars = ax4.bar(phases, overall_scores, color=['skyblue', 'lightcoral'], alpha=0.8)
        ...
        plt.tight_layout()
        plt.savefig(self.results_dir / 'phase4_vs_phase5_comparison.png', dpi=300, bbox_inches='tight')
        plt.close()
        
        logger.info(f"   Visualization saved: {self.results_dir / 'phase4_vs_phase5_comparison.png'}")
...
"""
        ...
        results_dict = {
            "phase4_results": {k: v.tolist() if isinstance(v, np.ndarray) else v 
                             for k, v in self.phase4_results.items()},
            "phase5_results": {k: v.tolist() if isinstance(v, np.ndarray) else v 
...
if __name__ == "__main__":
    ...
    print(f"\n📊 FINAL RESULTS:")
    print(f"Phase 4: Motor R² = {phase4['motor_r2']:.4f}, Cognitive AUC = {phase4['cognitive_auc']:.4f}")
    print(f"Phase 5: Motor R² = {phase5['motor_r2']:.4f}, Cognitive AUC = {phase5['cognitive_auc']:.4f}")
    print(f"\n🎯 IMPROVEMENTS:")
    print(f"Motor: {stats['motor_r2_improvement']:+.4f} ({stats['motor_r2_improvement_pct']:+.1f}%)")
    print(f"Cognitive: {stats['cognitive_auc_improvement']:+.4f} ({stats['cognitive_auc_improvement_pct']:+.1f}%)")
    print(f"Overall: {stats['overall_score_improvement']:+.4f}")
    
    print(f"\n🏆 Best Dynamic Loss Strategy:")
    best_strategy = max(dynamic_results.keys(), key=lambda k: dynamic_results[k]['cognitive_auc'])
    best_results = dynamic_results[best_strategy]
    print(f"{best_strategy.upper()}: Motor R² = {best_results['motor_r2']:.4f}, Cognitive AUC = {best_results['cognitive_auc']:.4f}")
