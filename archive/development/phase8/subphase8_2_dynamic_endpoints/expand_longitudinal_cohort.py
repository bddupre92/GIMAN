...
def load_baseline_cohort() -> pd.DataFrame:
    ...
    cohort_path = (
        ...
        if cohort_override
        else project_root / "data" / "prodromal_cohort" / "prodromal_survival_data.csv"
    )
...
def load_longitudinal_clinical_data() -> pd.DataFrame:
    ...
    resolved = resolver.resolve_latest(
        "updrs_part_iii_longitudinal",
        ["MDS-UPDRS_Part_III_*.csv"],
        required=True,
        allow_empty=False,
        required_columns=["PATNO", "EVENT_ID", "NP3TOT"],
    )
    if resolved is None:
        raise RuntimeError("Failed to resolve longitudinal UPDRS source.")
    updrs_path = Path(resolved.path)

    updrs = pd.read_csv(updrs_path)
    print(f"✓ Longitudinal UPDRS source: {updrs_path}")
...
def create_landmark_observations(
    ...
    for landmark_month in landmark_times:
        ...
        for _, patient in merged.iterrows():
            ...
            if landmark_month == 0:
                ...
                features = patient.copy()
                features["baseline_updrs"] = longitudinal_at_landmark.iloc[0][
                    "updrs_total"
                ]
            ...
            feature_cols = [
                ...
                if c not in ["PATNO", "time_to_event", "phenoconverted"]
            ]
    ...
    for lm in landmark_times:
        ...
        print(
            f"  {int(lm):2d} months: {len(lm_data):4d} obs, {int(lm_events):2d} events"
        )
...
def validate_expansion(expanded_df: pd.DataFrame) -> None:
    ...
    print(
        f"✓ Feature completeness: {100 * (1 - missing / total):.1f}% ({missing}/{total} missing)"
    )
