...
def load_enhanced_giman_data() -> pd.DataFrame:
    ...
    try:
        enhanced_path = (
            "data/enhanced/enhanced_giman_12features_v1.1.0_20250924_075919.csv"
        )
        df = pd.read_csv(enhanced_path)
        print(
            f"✅ Enhanced GIMAN data loaded: {df.shape[0]} records, {df.shape[1]} features"
        )
...
def load_longitudinal_data() -> pd.DataFrame:
    ...
    try:
        ...
        print(
            f"✅ Longitudinal data loaded: {df.shape[0]} records, {df.shape[1]} features"
        )
        print(f"   Unique patients: {df['PATNO'].nunique()}")
        return df
    except FileNotFoundError:
...
def explore_progression_data(
    enhanced_df: pd.DataFrame, longitudinal_df: pd.DataFrame
) -> None:
    ...
    print("Motor Progression Analysis:")
    print(
        f"  • Enhanced patients in longitudinal data: {enhanced_longitudinal['PATNO'].nunique()}"
    )
    ...
    for patient in sample_patients:
        patient_data = motor_data[motor_data["PATNO"] == patient].sort_values(
            "EVENT_ID"
        )
        scores = patient_data["NP3TOT"].tolist()
        visits = len(scores)
        cohort = enhanced_df[enhanced_df["PATNO"] == patient]["COHORT_DEFINITION"].iloc[
            0
        ]
...
def calculate_motor_progression_slopes(
    ...
    print("✅ Motor progression slopes calculated:")
    print(f"   • Patients with slopes: {len(slopes_df)}")
    print(
        f"   • Mean slope: {slopes_df['motor_slope'].mean():.3f} ± {slopes_df['motor_slope'].std():.3f} points/month"
    )
    print(
        f"   • Slope range: {slopes_df['motor_slope'].min():.3f} to {slopes_df['motor_slope'].max():.3f}"
    )
...
def calculate_cognitive_conversion_labels(
    longitudinal_df: pd.DataFrame,
    enhanced_patients: list[int],
    time_window_months: int = 36,
    min_visits: int = 3,
    mci_threshold: float = 3.0,
    dementia_threshold: float = 5.0,
) -> pd.DataFrame:
    ...
    for patient in enhanced_patients:
        ...
        if len(patient_data) >= min_visits:
            ...
            mci_conversion = (max_decline >= mci_threshold) and (
                patient_data["MCATOT"].min() < 26
            )
            dementia_conversion = (max_decline >= dementia_threshold) and (
                patient_data["MCATOT"].min() < 24
            )
            ...
            conversions.append(
                {
                    ...
                    "months_observed": patient_data["MONTHS"].iloc[-1]
                    - patient_data["MONTHS"].iloc[0],
    ...
    print("✅ Cognitive conversion labels calculated:")
    print(f"   • Patients with labels: {len(conversions_df)}")
    print(
        f"   • Any conversion: {conversions_df['cognitive_conversion'].sum()} ({conversions_df['cognitive_conversion'].mean() * 100:.1f}%)"
    )
    print(
        f"   • MCI conversion: {conversions_df['mci_conversion'].sum()} ({conversions_df['mci_conversion'].mean() * 100:.1f}%)"
    )
    print(
        f"   • Dementia conversion: {conversions_df['dementia_conversion'].sum()} ({conversions_df['dementia_conversion'].mean() * 100:.1f}%)"
    )
    print(
        f"   • Mean max decline: {conversions_df['max_decline'].mean():.2f} ± {conversions_df['max_decline'].std():.2f} points"
    )
...
def create_prognostic_dataset(
    enhanced_df: pd.DataFrame,
    motor_slopes: pd.DataFrame,
    cognitive_labels: pd.DataFrame,
) -> Data:
    ...
    dataset = patient_features.merge(
        motor_slopes[["PATNO", "motor_slope"]], on="PATNO", how="left"
    )
    dataset = dataset.merge(
        cognitive_labels[["PATNO", "cognitive_conversion"]], on="PATNO", how="left"
    )
    ...
    motor_targets = torch.tensor(
        dataset["motor_slope"].fillna(0).values, dtype=torch.float32
    )
    cognitive_targets = torch.tensor(
        dataset["cognitive_conversion"].fillna(0).values, dtype=torch.float32
    )

    # Create masks for available targets
    motor_mask = torch.tensor(~dataset["motor_slope"].isna().values, dtype=torch.bool)
    cognitive_mask = torch.tensor(
        ~dataset["cognitive_conversion"].isna().values, dtype=torch.bool
    )
    ...
    nbrs = NearestNeighbors(n_neighbors=7, metric="cosine").fit(
        X_scaled
    )  # 7 to include self
    ...
    print("✅ Prognostic dataset created:")
    ...
    for epoch in range(epochs):
        ...
        if data.motor_mask.sum() > 0:
            motor_loss = F.mse_loss(
                motor_pred[data.motor_mask], data.motor_y[data.motor_mask]
            )
        else:
        ...
        if data.cognitive_mask.sum() > 0:
            cognitive_loss = F.binary_cross_entropy(
                cognitive_pred[data.cognitive_mask],
        ...
        if (epoch + 1) % 50 == 0:
            print(
                f"   Epoch {epoch + 1:3d}: Total={total_loss:.6f}, Motor={motor_loss:.6f}, Cognitive={cognitive_loss:.6f}"
            )
...
def evaluate_prognostic_model(model: PrognosticGIMAN, data: Data) -> dict:
    ...
    if data.motor_mask.sum() > 0:
        ...
        print("Motor Progression Regression:")
...
def create_prognostic_visualizations(
    data: Data,
    ...
    fig.suptitle(
        "GIMAN Phase 1 Prognostic Development - Analysis Results",
        fontsize=16,
        fontweight="bold",
    )
    ...
    corr = np.corrcoef(motor_slopes["baseline_updrs"], motor_slopes["motor_slope"])[
        0, 1
    ]
    axes[0, 2].text(
        0.05,
    ...
    if data.motor_mask.sum() > 0:
        ...
        min_val, max_val = (
            min(motor_true.min(), motor_predictions.min()),
            max(motor_true.max(), motor_predictions.max()),
        )
    ...
    for i, patient in enumerate(sample_patients):
        ...
        months = np.linspace(
            0, patient_data["months_observed"], int(patient_data["n_visits"])
        )
        predicted_progression = (
            patient_data["baseline_updrs"] + patient_data["motor_slope"] * months
        )
...
def main():
    ...
    cognitive_labels.to_csv(
        "data/prognostic/cognitive_conversion_labels.csv", index=False
    )
    ...
    print(
        f"   • Motor progression model R²: {results.get('motor', {}).get('r2', 'N/A')}"
    )
    print(
        f"   • Cognitive decline model AUC: {results.get('cognitive', {}).get('auc_roc', 'N/A')}"
    )
    print("   • Results saved to: data/prognostic/")
    print("   • Model saved to: models/prognostic_giman_phase1.pth")
