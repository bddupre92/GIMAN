...
def load_enhanced_graph_data() -> Data:
    ...
    logger.info("✅ Enhanced graph data loaded:")
...
def create_enhanced_config() -> dict[str, Any]:
    ...
    enhanced_config["training_params"]["max_epochs"] = (
        150  # More epochs for new features
    )
    enhanced_config["training_params"]["patience"] = 15  # Slightly more patience

    logger.info("🔧 Enhanced Configuration Created:")
    logger.info(
        f"   📊 Input Features: 7 → 12 ({enhanced_config['model_params']['input_dim']})"
    )
    logger.info(f"   🏗️  Architecture: {enhanced_config['model_params']['hidden_dims']}")
    logger.info(
        f"   🔗 Graph: k={enhanced_config['graph_params']['top_k_connections']} cosine similarity"
    )
    logger.info(
        f"   🎯 Loss: Focal(α={enhanced_config['loss_params']['focal_alpha']}, γ={enhanced_config['loss_params']['focal_gamma']})"
    )
...
def create_data_splits(
    graph_data: Data, config: dict[str, Any]
...
def train_enhanced_model() -> dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"   📊 Input Dimension: {config['model_params']['input_dim']} features"
        )
        ...
        logger.info(
            f"📊 Improvement from v1.0.0: {test_results['auc_roc'] - 0.9893:+.4f} AUC-ROC"
        )
