...
\caption[Head-to-head $C$-index]{Paired-bootstrap $C$-index on time-to-NP4OFF-$\geq 1$ (626 shared-cohort patients, 480 events). (A)~Bootstrap distribution of $C$-indices for the mechanistic twin (\texttt{pct\_loss\_per\_yr}) and Graph-DT (sum of 5-yr CIF). Both distributions straddle 0.5. (B)~Paired $\Delta$($C$-index) distribution is shifted mildly negative ($-0.047$, paired $p{=}0.046$), consistent with wearing-off being a PK-driven endpoint that the mechanistic twin's neurodegeneration-rate signal does not capture.}
...
\caption[Observational counterfactual scatter]{Observed versus predicted $\Delta$gap at 481 LEDD-escalation events ($\geq 200$~mg) across 335 PPMI patients. Predictions are from the Phase~4 Path~B severity-controlled interaction model without retuning. Dashed line is $y = x$; solid line is the paired-bootstrap regression fit (slope $1.074$, 95\% CI $[0.877, 1.285]$).}
\label{fig:p10-counterfactual}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{p10_fig8_calibration_bins.pdf}
\caption[Counterfactual calibration bins]{Decile-binned calibration of predicted against observed $\Delta$gap. Bin means lie close to the identity line across the full prediction range (including large-escalation tails), with bootstrap 95\% CIs overlapping identity in every bin. Consistent with the tabulated slope of 1.07, this is a strong positive for the NASEM control-as-purpose criterion.}
\label{fig:p10-calibration}
\end{figure}
...
\begin{table}[H]
\centering
\caption{Comparison with prior PD mechanistic digital twins, situated in the NASEM~2024 maturity framework.}
...
\midrule
V\'{e}ronneau 2020~\cite{veronneau2020}            & No             & No              & No       & one-shot-fit \\
...
\textbf{This work (PD)}                             & \textbf{Yes (SIR episodic)} & \textbf{Cross-sectional LCC} & \textbf{Formal 16/21} & \textbf{bidirectional-ready} \\
\bottomrule
\end{tabular}
\end{table}
