...
\paragraph*{Interpretability claims scope (R3-6)} The Graph-DT's "patient-similarity-based interpretability" is positioned in this paper as a \emph{methodological} contribution (the architecture surfaces nearest-neighbour trajectories in addition to point predictions), not as a \emph{clinically validated} interpretability gain. We do not perform a clinician reader study, do not compute quantitative interpretability metrics (e.g., per-patient SHAP attributions on Graph-DT vs.\ DeepHit), and do not demonstrate that the nearest-neighbour overlays positively alter clinical decision-making. The interpretability claim should therefore be read as "the model exposes a population-context view that DeepHit does not," not as "this view has been shown to improve clinical reasoning." A formal clinician-in-the-loop evaluation (WS-P3-15 faithfulness metrics + future reader study) is required to upgrade this from a methodological claim to a clinical-utility claim.
