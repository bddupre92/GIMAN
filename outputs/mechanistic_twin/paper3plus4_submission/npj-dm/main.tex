...
% ================================================================
\begin{abstract}
\noindent The Neuronal alpha-Synuclein Disease Integrated Staging System (NSD-ISS) provides a biologically-grounded framework for Parkinson's disease (PD), but predictive models for stage-transition timing with calibrated uncertainty do not yet exist. Using longitudinal data from 1{,}900 patients in the Parkinson's Progression Markers Initiative (16{,}699 visits, 2{,}859 observed transitions), we develop a Graph-Regularized Transition-Timing Model (Graph-DT) that combines a gated recurrent temporal encoder with graph attention over a patient-similarity graph, and benchmark it against Dynamic-DeepHit and a multi-state Markov baseline. In 5-fold stratified cross-validation, Dynamic-DeepHit and Graph-DT achieve time-dependent concordance of $0.924 \pm 0.020$ and $0.904 \pm 0.034$ (paired $\Delta C_\text{td} = -0.020$); Graph-DT contributes discrimination stability (28\% lower fold variance) and population-level regularisation rather than per-patient attribution. Both models are extended with cause-specific conformal CIF bands using inverse probability of censoring weighting (IPCW), achieving marginal coverage of 95.1\%/95.3\% at 95\% and 90.2\%/90.5\% at 90\%. Expected calibration error remains below 0.012 at 1-, 3-, and 5-year horizons. Subgroup analysis finds no significant model-by-subgroup interactions across sex, age, or genetic strata (LRRK2, GBA, APOE~$\varepsilon4$; FDR-corrected); Mondrian per-stratum recalibration confirms equity across all eight (model$\times$stratum) cells. Individual case studies yield interpretable transition-timing intervals of 14--29 months for the most frequent transitions.
\end{abstract}
...
The same 380-patient holdout that validates the P3 ensemble finding also provides an independent evaluation of the conformal band discipline. Using the dev-set 50/50 calibration split (seed~=~2026) under the corrected IPCW pipeline (commit \texttt{f8a7c54}; Methods~\S Cause-specific conformal CIF bands with IPCW; Supplementary~\S S-CRIT-A), marginal coverage at 95\% confidence is 0.949 for DeepHit and 0.956 for Graph-DT, matching the 5-fold CV means in Table~\ref{tab:conformal_cov} (0.951 and 0.953) within $\pm 0.005$. At 90\% confidence the holdout achieves 0.896 (DeepHit) and 0.903 (Graph-DT), again within $\pm 0.01$ of the CV means (0.902 and 0.905). Subgroup coverage stratified by sex and age bin at 90\% CL is 0.890--0.904 (DeepHit) and 0.890--0.908 (Graph-DT) --- all within 0.012 of the marginal holdout figure, supporting the equity claim made in the primary analysis.
...
All results derive from PPMI; external validation on independent cohorts with different enrolment criteria remains essential, but is currently data-blocked (Supplementary~\S~S-WS-P3-16) by the absence of the full 18-feature input vector across the available AMP-PD external cohorts. Finally, the 85.4\% coverage for backward transitions falls below the nominal 90\% target, indicating that conformal guarantees may not fully hold for treatment-driven transitions without treatment-aware modelling.
