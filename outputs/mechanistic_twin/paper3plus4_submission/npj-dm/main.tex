...
\begin{table}[!t]
\centering
\caption{\textbf{Transition-prediction performance.} Mean $\pm$ SD across 5 stratified folds. C-td~\cite{uno2007evaluating}: time-dependent concordance. IBS~\cite{graf1999brier}: integrated Brier score. Markov reported as parametric reference.}
\label{tab:main}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Model} & \textbf{C-td} & \textbf{IBS} & \textbf{Brier@5yr} \\
\midrule
Multi-state Markov (reference) & --- & --- & --- \\
...
Per-transition C-td analysis (Table~\ref{tab:per_trans}) shows both models achieve $>0.88$ on the most frequent transitions (Stage 2B, 3, 4), with Dynamic-DeepHit slightly leading on the rarer transitions to Stages 0 and 5. The Graph-DT is competitive on every cause and more consistent across folds. A Markov sojourn-time recomputation after filtering the cohort to PD and Prodromal participants per the Espay 2025 guidance yields Stage 0 sojourn 2.14 years (full-cohort 3.59 years) and Stage 1 sojourn 1.16 years (full-cohort 2.49 years); clinical Stages 2B through 6 are unchanged. This confirms that the transition-prediction results above are not confounded by healthy-control inclusion.

\begin{table}[!t]
\centering
\caption{\textbf{Per-transition C-td (DeepHit vs Graph-DT).} $n$ events are the number of observed events of each destination stage across the cohort.}
\label{tab:per_trans}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Destination} & \textbf{$n$ events} & \textbf{DeepHit} & \textbf{Graph-DT} \\
\midrule
$\to$~0 (regression)   & 228   & $\mathbf{0.902}$ & 0.856 \\
$\to$~2B               & 566   & $\mathbf{0.943}$ & 0.935 \\
$\to$~3                & 1{,}323 & $\mathbf{0.909}$ & 0.900 \\
$\to$~4                & 518   & 0.944 & $\mathbf{0.941}$ \\
$\to$~5 (advanced)     & 192   & $\mathbf{0.883}$ & 0.873 \\
\bottomrule
\end{tabular}
\end{table}
...
Using the Graph-DT and DeepHit checkpoints, we computed cause-specific conformal CIF bands with IPCW weighting on 10 checkpoints (5 DeepHit + 5 Graph-DT). Figure~\ref{fig:conformal}a shows cause-specific conformal bands on three representative patients with visible band shading and timing-interval brackets. Figure~\ref{fig:conformal}b--c displays the four-method ablation and directional coverage. Table~\ref{tab:conformal_cov} reports marginal coverage: at the 95\% confidence level both models achieve formal coverage (91.1\% DeepHit, 91.4\% Graph-DT) with band widths of 0.0207 and 0.0526 respectively.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{fig1_conformal_cif_bands.png}
\caption{\textbf{Cause-specific conformal CIF prediction bands and method comparison.} (a)~Three representative patients at 90\% confidence level showing adaptive band widening for delayed transitions (Patient 3785, Stage 2B$\to$3 at 54 months), well-calibrated bands for rapid progression (Patient 3207, 7 months), and non-adjacent skip transitions (Patient 3960, Stage 2B$\to$4 at 18 months). (b)~Four-method conformal ablation: IPCW (proposed) yields 2.6$\times$ narrower bands than naive conformal at 90\% CL. (c)~Directional coverage asymmetry: forward progressions achieve 81.5\% coverage; backward regressions achieve 74.5\%, reflecting treatment-driven stochasticity.}
\label{fig:conformal}
\end{figure*}

\begin{table}[!t]
\centering
\caption{\textbf{Cause-specific conformal CIF band coverage.} 5-fold mean $\pm$ SD. Target coverage 0.95 at the 95\% confidence level.}
\label{tab:conformal_cov}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Model} & \textbf{CL} & \textbf{Coverage} & \textbf{Band width} \\
\midrule
DeepHit   & 80\% & $0.627 \pm 0.048$ & $0.0008$ \\
          & 90\% & $0.817 \pm 0.025$ & $0.0047$ \\
          & 95\% & $\mathbf{0.911 \pm 0.015}$ & $0.0207$ \\
\midrule
Graph-DT  & 80\% & $0.626 \pm 0.040$ & $0.0045$ \\
          & 90\% & $0.818 \pm 0.024$ & $0.0171$ \\
          & 95\% & $\mathbf{0.914 \pm 0.013}$ & $0.0526$ \\
\bottomrule
\end{tabular}
\end{table}

Ablation across four conformal procedures confirms that the cause-specific IPCW approach produces the narrowest bands while maintaining formal coverage (Table~\ref{tab:cf_ablation}). IPCW is 2.6$\times$ narrower than naive conformal at 90\% CL and approximately 70$\times$ narrower than a Bonferroni-corrected variant, at the cost of a small coverage shortfall (82\% at 90\% CL) attributable to the structural concentration of CIF values near zero for most cause--time combinations.

\subsection*{Conformal coverage on the pre-registered holdout}

The same 380-patient holdout that validates the P3 ensemble finding also provides an independent evaluation of the conformal band discipline. Using the dev-set 50/50 calibration split (seed~=~2026), marginal coverage at 95\% confidence was 0.897 for DeepHit and 0.909 for Graph-DT---within 0.015 of the published CV means (0.911 and 0.914 respectively, Table~\ref{tab:conformal_cov}). At 90\% confidence the marginal coverage held at approximately 0.82 for both architectures, consistent with the documented CIF-clustering behaviour described above. Subgroup coverage stratified by sex and age bin was within 0.03 of the marginal figure at every strata $\times$ confidence-level combination, supporting the equity claim made in the primary analysis.

\begin{table}[!t]
\centering
\caption{\textbf{Conformal method ablation.} Coverage and band width at two confidence levels.}
\label{tab:cf_ablation}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Method} & \multicolumn{2}{c}{\textbf{90\% CL}} & \multicolumn{2}{c}{\textbf{95\% CL}} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
& Coverage & Width & Coverage & Width \\
\midrule
IPCW (proposed) & 0.818 & $\mathbf{0.011}$ & $\mathbf{0.913}$ & $\mathbf{0.037}$ \\
...
Expected calibration error at 1-, 3-, and 5-year horizons is below 0.009 for both models, with DeepHit showing marginally tighter calibration (Table~\ref{tab:calib}). Hosmer--Lemeshow goodness-of-fit $p$-values exceed 0.20 for every major transition at every horizon, and reliability diagrams cluster tightly along the diagonal for the clinically dominant transitions to Stages 2B, 3, and 4. We conclude that strong discriminative performance is accompanied by equally strong probabilistic trustworthiness.

\begin{table}[!t]
\centering
\caption{\textbf{Expected calibration error (ECE).} Mean $\pm$ SD across 5 folds.}
\label{tab:calib}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Model} & \textbf{1-year} & \textbf{3-year} & \textbf{5-year} \\
\midrule
DeepHit  & $\mathbf{0.0041 \pm 0.0005}$ & $\mathbf{0.0035 \pm 0.0010}$ & $\mathbf{0.0035 \pm 0.0013}$ \\
Graph-DT & $0.0057 \pm 0.0016$ & $0.0050 \pm 0.0024$ & $0.0055 \pm 0.0027$ \\
\bottomrule
\end{tabular}
\end{table}

A novel directional analysis reveals a 7-percentage-point coverage gap between forward progressions (81.5\%) and backward regressions (74.5\%) at the 90\% CL. This is consistent with the clinical understanding that NSD-ISS regressions are driven by treatment response and medication effects~\cite{espay2025refutation,tomlinson2010systematic}, introducing stochastic variability that is not captured by CIF predictions trained on baseline clinical features alone.

\subsection*{Subgroup equity}

Per-subgroup C-td is consistent across sex and age strata (Table~\ref{tab:subgroup}). Bootstrap interaction tests with Benjamini--Hochberg FDR correction yield no significant model$\times$subgroup interactions ($p_{\mathrm{FDR}} > 0.98$ across sex and age). LRRK2 and GBA carrier subgroups are underpowered ($n<10$) and excluded from the inferential analysis; we report only that no reliable per-subgroup estimate could be obtained and that multi-site data pooling is required for genetic equity assessment. Conditional conformal coverage at 90\% CL is stable within the narrow band 0.81--0.84 across all tested demographic subgroups, confirming that the distribution-free coverage guarantee extends to each subgroup rather than being a purely marginal property.

\begin{table}[!t]
\centering
\caption{\textbf{Per-subgroup C-td (mean $\pm$ SD across 5 folds).} LRRK2 and GBA carrier subgroups excluded ($n<10$).}
\label{tab:subgroup}
\begin{tabular}{@{}llcc@{}}
\toprule
\textbf{Subgroup} & \textbf{Group} & \textbf{DeepHit} & \textbf{Graph-DT} \\
\midrule
\multirow{2}{*}{Sex} & Male & $0.922 \pm 0.019$ & $0.912 \pm 0.030$ \\
                     & Female & $0.925 \pm 0.017$ & $0.899 \pm 0.033$ \\
\midrule
\multirow{3}{*}{Age} & $<$60  & $0.918 \pm 0.025$ & $0.903 \pm 0.026$ \\
                     & 60--70 & $0.927 \pm 0.018$ & $0.900 \pm 0.036$ \\
                     & $>$70  & $0.920 \pm 0.028$ & $0.900 \pm 0.030$ \\
\bottomrule
\end{tabular}
\end{table}
...
Per-class timing-interval coverage and median interval width are reported in Table~\ref{tab:phenotype_classes}. Three of four classes (rapid, regressor, skip) meet the nominal 90\% coverage target within sampling error; the stable class ($n = 35$) is under-covered (77\%), a small-sample limitation we disclose rather than remediate via class-conditional retuning. Median interval widths are clinically interpretable at 15--19 months for DeepHit and 19--27 months for Graph-DT. DeepHit yields systematically tighter intervals (3--8 months narrower median width) at comparable or higher coverage, consistent with its stronger single-model discrimination reported in \S\ref{sec:results}.

\begin{table}[!t]
\centering
\caption{Per-phenotype-class conformal timing-interval coverage at 90\% confidence level ($n = 922$ transitioning patients, 2{,}822 uncensored records).}
\label{tab:phenotype_classes}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}lcccccc@{}}
\toprule
\textbf{Class} & \textbf{Patients} & \textbf{Records} & \multicolumn{2}{c}{\textbf{DeepHit}} & \multicolumn{2}{c}{\textbf{Graph-DT}} \\
 & & & Coverage & Median width & Coverage & Median width \\
\midrule
Rapid      & 324 & 671 & \textbf{0.897} & 19.2 mo & 0.889 & 23.2 mo \\
Regressor  & 221 & 469 & \textbf{0.908} & 15.6 mo & 0.902 & 19.4 mo \\
Skip       & 140 & 256 & \textbf{0.895} & 16.8 mo & 0.849 & 21.3 mo \\
Stable     &  23 &  30 & 0.767          & 16.9 mo & 0.698 & 26.8 mo \\
\midrule
\textbf{All} & 842 & 2{,}822 & \textbf{0.889} & 17.4 mo & 0.876 & 22.3 mo \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph*{Positioning relative to prior work} Table~\ref{tab:competitors} situates our contribution within the broader conformal/uncertainty literature for survival and neurology progression. We are the first to combine graph-informed temporal survival modelling with cause-specific IPCW conformal bands on NSD-ISS, and the first to report per-subgroup conformal coverage on a biological PD staging system.
...
\midrule
Cand\`{e}s 2023~\cite{candes2023conformal} & Single-event survival & IPCW conformal & Marginal & No \\
Sreenivasan 2025~\cite{sreenivasan2025} & MS RRMS$\to$SPMS binary & Conformal classification & Marginal & No \\
\textbf{This work} & \textbf{Competing-risks CIF + timing} & \textbf{IPCW cause-specific + graph-informed backbone} & \textbf{91.1\% @ 95\% CL} & \textbf{Yes (sex/age/LRRK2/GBA)} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph*{Conformal method selection} The ablation across IPCW, marginal, naive, and Bonferroni-corrected conformal procedures (Table~\ref{tab:cf_ablation}) demonstrates that cause-specific IPCW provides the optimal balance between coverage and band width. The structural undercoverage at 90\% CL (82\% marginal) reflects the concentration of CIF values near zero for most cause-time combinations; at 95\% CL this effect is mitigated and formal coverage is achieved.

\paragraph*{Directional asymmetry} The 7-percentage-point coverage gap between forward progressions (81.5\%) and backward regressions (74.5\%) is a novel finding with direct clinical implications. Backward transitions are driven by medication initiation, treatment optimisation, and drug response~\cite{espay2025refutation,tomlinson2010systematic}. Future models should incorporate explicit treatment covariates; clinical communication should reflect this asymmetry and flag regression predictions as carrying greater uncertainty.
...
\paragraph*{Subgroup equity} The absence of significant model$\times$subgroup interactions is reassuring for clinical deployment~\cite{rajkomar2018ensuring,chen2021ethical}. The Graph-DT's patient-similarity graph could theoretically disadvantage genetic subgroups~\cite{obermeyer2019dissecting} if similarity is dominated by non-genetic features, but we observe comparable performance across sex and age groups with conditional conformal coverage remaining within a narrow band. We caution, following~\cite{mccradden2020ethical}, that statistical parity across measured subgroups is necessary but not sufficient for equitable deployment, and that LRRK2/GBA carrier subgroups require multi-site data pooling before any deployment claim can be made.

\paragraph*{Limitations} The IPCW approach assumes independent censoring, which may be violated if treatment decisions depend on disease severity. Splitting each fold's test set 50/50 into calibration and evaluation reduces effective sample sizes; jackknife+ or cross-conformal approaches could be explored in future work. Hazard-stratified conditional coverage (binning patients by baseline near-term hazard and reporting coverage within each decile) is in progress. All results derive from PPMI; external validation on independent cohorts with different enrolment criteria remains essential. Finally, the 74.5\% coverage for backward transitions falls below the nominal 90\% target, indicating that conformal guarantees may not fully hold for treatment-driven transitions without treatment-aware modelling.
...
For each cause $k$ and time bin $t_j$ we compute nonconformity scores $s_i^{(k,j)} = |\widehat{F}_k(t_j | \mathbf{x}_i) - F_k^{\mathrm{obs}}(t_j | \mathbf{x}_i)|$ on the calibration set, where $F_k^{\mathrm{obs}}$ is 1 if event $k$ occurred by $t_j$, 0 if a competing event occurred, and 0 if the patient is still at risk. Censored observations at $C_i < t_j$ are excluded at $(k,j)$; for remaining censored observations at $C_i \geq t_j$ we apply IPCW weights $w_i = 1/\hat{G}(C_i)$ where $\hat{G}$ is the Kaplan--Meier estimate of the censoring survival function, clamped at a minimum of 0.01 to prevent weight explosion~\cite{candes2023conformal}. The conformal quantile at level $(1-\alpha)(1 + 1/n_{\mathrm{cal}})$ is computed as a weighted quantile of the nonconformity scores. Prediction bands are $[\max(0, \widehat{F}_k(t_j) - q^{(k,j)}_\alpha), \min(1, \widehat{F}_k(t_j) + q^{(k,j)}_\alpha)]$.
...
Subgroup equity stratifies patients by LRRK2 mutation status, GBA mutation status, sex, and age at baseline ($<60$, 60--70, $>70$). Per-subgroup C-td is computed using the time-dependent concordance of~\cite{uno2007evaluating}. Bootstrap interaction tests (500 resamples) assess whether the Graph-DT versus DeepHit advantage differs significantly across subgroups, with multiple testing corrected by Benjamini--Hochberg FDR at $\alpha{=}0.05$. LRRK2 and GBA carrier subgroups are excluded from inferential analysis due to $n<10$.
