...
def main():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Research analytics not fully available due to missing dependencies: {e}")
        ...
        if spatial_emb is None or genomic_emb is None:
            logger.error("Failed to generate embeddings. Using synthetic data for testing.")
        ...
        temporal_emb = np.random.randn(spatial_emb.shape[0], 256)  # Placeholder for temporal features
        ...
        if prognostic_data is None:
            logger.warning("No prognostic data available. Creating synthetic targets for testing.")
            n_patients = spatial_emb.shape[0]
            motor_scores = np.random.randn(n_patients) * 2.0  # Synthetic motor progression scores
            cognitive_conversion = np.random.binomial(1, 0.3, n_patients)  # 30% conversion rate
        else:
        ...
        # Training setup
        optimizer = torch.optim.AdamW(unified_system.parameters(), lr=0.001, weight_decay=0.01)
            ...
            if epoch % 10 == 0:
                ...
                with torch.no_grad():
                    ...
                    val_motor_loss = torch.nn.functional.huber_loss(
                        ...
                        motor_tensor[val_idx]
                    )
                    
                    val_cognitive_loss = torch.nn.functional.binary_cross_entropy_with_logits(
        ...
        with torch.no_grad():
            ...
            cognitive_pred = torch.sigmoid(test_outputs['cognitive_prediction'].squeeze()).cpu().numpy()
        ...
        if research_analytics_available:
            ...
            publication_analysis = research_analytics.generate_publication_ready_analysis(training_results)
            ...
            logger.info("📊 Creating basic analysis (without advanced interpretability)...")
            ...
            ax1.plot([min(motor_true), max(motor_true)], [min(motor_true), max(motor_true)], 'r--')
            ...
            ax3.bar(x + width/2, cognitive_auc_values, width, label='Cognitive AUC', alpha=0.8)
...
"""
            ...
            plt.suptitle('GIMAN Phase 4: Unified System Analysis', fontsize=16, fontweight='bold')
            plt.tight_layout()
            plt.savefig(results_dir / 'phase4_analysis.png', dpi=300, bbox_inches='tight')
            plt.close()
