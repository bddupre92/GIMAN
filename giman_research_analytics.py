...
from sklearn.metrics import r2_score, roc_auc_score, accuracy_score, precision_recall_curve
...
class GIMANResearchAnalytics:
    ...
    def run_statistical_significance_tests(self, training_results: Dict) -> Dict:
        ...
        logger.info(f"📈 Motor correlation: {motor_correlation:.4f} (p={motor_p_value:.4f})")
        logger.info(f"🧠 Cognitive AUC: {cognitive_auc:.4f} CI: [{cognitive_auc_ci[0]:.3f}, {cognitive_auc_ci[1]:.3f}]")
    ...
    def analyze_feature_importance(self, training_results: Dict) -> Dict:
        ...
        with torch.no_grad():
            spatial_tensor = torch.tensor(spatial_emb, dtype=torch.float32).to(self.analyzer.device)
            genomic_tensor = torch.tensor(genomic_emb, dtype=torch.float32).to(self.analyzer.device)
            temporal_tensor = torch.tensor(temporal_emb, dtype=torch.float32).to(self.analyzer.device)
            
            outputs = self.unified_system(spatial_tensor, genomic_tensor, temporal_tensor)
            
            feature_importance_weights = outputs['feature_importance'].cpu().numpy()
            attention_importance = outputs['attention_output']['attention_importance'].cpu().numpy()
        ...
        modality_importance = {
            'spatial': {
                ...
                'perm_motor': np.mean(motor_perm_importance.importances_mean[:n_spatial]),
                'perm_cognitive': np.mean(cognitive_perm_importance.importances_mean[:n_spatial]),
                'neural_attention': np.mean(feature_importance_weights[:, :n_spatial])
            },
            'genomic': {
                'rf_motor': np.mean(rf_motor.feature_importances_[n_spatial:n_spatial+n_genomic]),
                'rf_cognitive': np.mean(rf_cognitive.feature_importances_[n_spatial:n_spatial+n_genomic]),
                'perm_motor': np.mean(motor_perm_importance.importances_mean[n_spatial:n_spatial+n_genomic]),
                'perm_cognitive': np.mean(cognitive_perm_importance.importances_mean[n_spatial:n_spatial+n_genomic]),
                'neural_attention': np.mean(feature_importance_weights[:, n_spatial:n_spatial+n_genomic])
            },
            'temporal': {
                'rf_motor': np.mean(rf_motor.feature_importances_[n_spatial+n_genomic:]),
                'rf_cognitive': np.mean(rf_cognitive.feature_importances_[n_spatial+n_genomic:]),
                'perm_motor': np.mean(motor_perm_importance.importances_mean[n_spatial+n_genomic:]),
                'perm_cognitive': np.mean(cognitive_perm_importance.importances_mean[n_spatial+n_genomic:]),
                'neural_attention': np.mean(feature_importance_weights[:, n_spatial+n_genomic:])
            }
        }
    ...
    def generate_counterfactuals(self, training_results: Dict, n_counterfactuals: int = 100) -> Dict:
        ...
        cf_indices = np.random.choice(test_idx, min(n_counterfactuals, len(test_idx)), replace=False)
        
        spatial_emb = torch.tensor(self.analyzer.spatiotemporal_embeddings[cf_indices], dtype=torch.float32)
        genomic_emb = torch.tensor(self.analyzer.genomic_embeddings[cf_indices], dtype=torch.float32)
        temporal_emb = torch.tensor(self.analyzer.temporal_embeddings[cf_indices], dtype=torch.float32)
        ...
        self.unified_system.eval()
        with torch.no_grad():
            original_outputs = self.unified_system(spatial_emb, genomic_emb, temporal_emb)
        ...
        for risk_multiplier in [0.5, 1.5, 2.0]:  # Reduce/increase genetic risk
            ...
            with torch.no_grad():
                ...
                cf_targets[:, 1] = (risk_multiplier - 1.0) * 0.5  # Cognitive target modification
                
                cf_outputs = self.unified_system(
                    spatial_emb, modified_genomic, temporal_emb,
        ...
        for imaging_change in [-0.2, -0.1, 0.1, 0.2]:  # Imaging improvement/decline
            ...
            with torch.no_grad():
                ...
                cf_targets[:, 1] = max(0, -imaging_change)  # Cognitive target (improvement reduces risk)
                
                cf_outputs = self.unified_system(
                    modified_spatial, genomic_emb, temporal_emb,
                    generate_counterfactuals=True,
                    counterfactual_targets=cf_targets
                )
        ...
        with torch.no_grad():
            ...
            optimal_outputs = self.unified_system(
                optimal_spatial, optimal_genomic, temporal_emb,
                generate_counterfactuals=True,
                counterfactual_targets=cf_targets
            )
            
            optimal_motor = optimal_outputs['counterfactual_motor'].cpu().numpy()
            optimal_cognitive = optimal_outputs['counterfactual_cognitive'].cpu().numpy()
        ...
        for scenario in counterfactual_scenarios:
            ...
            motor_clinically_significant = np.mean(np.abs(scenario['motor_change']) > 0.1)
            cognitive_clinically_significant = np.mean(np.abs(scenario['cognitive_change']) > 0.1)
    ...
    def create_research_visualizations(self, training_results: Dict):
        ...
        phases = ['Phase 3.1', 'Phase 3.2\n(Original)', 'Phase 3.2\n(Improved)', 'Phase 4\n(Unified)']
        motor_r2_values = [-0.6481, -1.4432, -0.0760, training_results['test_metrics']['motor_r2']]
        cognitive_auc_values = [0.4417, 0.5333, 0.7647, training_results['test_metrics']['cognitive_auc']]
        
        x = np.arange(len(phases))
        width = 0.35
        
        bars1 = ax_perf.bar(x - width/2, motor_r2_values, width, label='Motor R²', alpha=0.8, color='skyblue')
        bars2 = ax_perf.bar(x + width/2, cognitive_auc_values, width, label='Cognitive AUC', alpha=0.8, color='lightcoral')
        ...
        if hasattr(self, 'statistical_tests'):
            motor_r2_dist = self.statistical_tests['bootstrap_results']['motor_r2_distribution']
            cognitive_auc_dist = self.statistical_tests['bootstrap_results']['cognitive_auc_distribution']
            
            ax_bootstrap.hist(motor_r2_dist, bins=30, alpha=0.7, label='Motor R²', density=True, color='skyblue')
            ax_bootstrap.hist(cognitive_auc_dist, bins=30, alpha=0.7, label='Cognitive AUC', density=True, color='lightcoral')
            ax_bootstrap.axvline(training_results['test_metrics']['motor_r2'], color='blue', linestyle='--', label='Observed Motor R²')
            ax_bootstrap.axvline(training_results['test_metrics']['cognitive_auc'], color='red', linestyle='--', label='Observed Cognitive AUC')
...
"""
        ...
        if hasattr(self, 'feature_importance_results'):
            modalities = ['Spatial\n(Imaging)', 'Genomic\n(Genetic)', 'Temporal\n(Longitudinal)']
            motor_importance = [
                self.feature_importance_results['modality_importance']['spatial']['neural_attention'],
                self.feature_importance_results['modality_importance']['genomic']['neural_attention'],
                self.feature_importance_results['modality_importance']['temporal']['neural_attention']
            ]
            cognitive_importance = [
                self.feature_importance_results['modality_importance']['spatial']['perm_cognitive'],
                self.feature_importance_results['modality_importance']['genomic']['perm_cognitive'],
                self.feature_importance_results['modality_importance']['temporal']['perm_cognitive']
            ]
            
            x = np.arange(len(modalities))
            width = 0.35
            
            ax_modality.bar(x - width/2, motor_importance, width, label='Motor Prediction', alpha=0.8, color='skyblue')
            ax_modality.bar(x + width/2, cognitive_importance, width, label='Cognitive Prediction', alpha=0.8, color='lightcoral')
        ...
        if hasattr(self, 'feature_importance_results') and 'shap_analysis' in self.feature_importance_results:
            shap_values = self.feature_importance_results['shap_analysis']['motor_shap_values']
            # Create a simplified SHAP-like visualization
            feature_importance = np.mean(np.abs(shap_values), axis=0)
            top_features = np.argsort(feature_importance)[-20:]  # Top 20 features
            
            y_pos = np.arange(len(top_features))
            ax_shap.barh(y_pos, feature_importance[top_features], color='green', alpha=0.7)
        ...
        if hasattr(self, 'counterfactual_results'):
            ...
            motor_effects = [self.counterfactual_results['effect_analysis'][s]['motor_effect_size'] for s in scenarios]
            cognitive_effects = [self.counterfactual_results['effect_analysis'][s]['cognitive_effect_size'] for s in scenarios]
            
            x = np.arange(len(scenarios))
            width = 0.35
            
            ax_cf_effects.bar(x - width/2, motor_effects, width, label='Motor Effect Size', alpha=0.8, color='skyblue')
            ax_cf_effects.bar(x + width/2, cognitive_effects, width, label='Cognitive Effect Size', alpha=0.8, color='lightcoral')
            ...
            ax_cf_effects.set_xticklabels([s.replace('_', '\n') for s in scenarios], rotation=45, ha='right')
        ...
        if hasattr(self, 'counterfactual_results'):
            ...
            if optimal_scenario:
                ...
                # Add quadrant labels
                ax_cf_dist.text(0.02, 0.98, 'Improved Motor\nReduced Cognitive Risk', 
                              transform=ax_cf_dist.transAxes, ha='left', va='top',
        ...
        ax_motor_clinical.scatter(motor_true, motor_pred, alpha=0.6, s=30, color='skyblue')
        ax_motor_clinical.plot([min(motor_true), max(motor_true)], [min(motor_true), max(motor_true)], 'r--', alpha=0.8)
        
        # Add clinical significance regions
        ax_motor_clinical.axhspan(-1, 1, alpha=0.2, color='green', label='Stable Range')
        ax_motor_clinical.axhspan(1, 5, alpha=0.2, color='yellow', label='Moderate Progression')
        ax_motor_clinical.axhspan(5, max(max(motor_true), max(motor_pred)), alpha=0.2, color='red', label='Rapid Progression')
        
        ax_motor_clinical.set_xlabel('True Motor Progression')
        ax_motor_clinical.set_ylabel('Predicted Motor Progression')
        ax_motor_clinical.set_title('Motor Prediction with Clinical Significance Regions')
        ...
        ax_cognitive_clinical.plot(recall, precision, color='blue', alpha=0.8, linewidth=2)
        ax_cognitive_clinical.fill_between(recall, precision, alpha=0.3, color='blue')
        ax_cognitive_clinical.axhline(y=np.sum(cognitive_true)/len(cognitive_true), color='red', linestyle='--', alpha=0.8, label='Baseline')
...
"""
        ...
        plt.suptitle('GIMAN Phase 4: Comprehensive Research Analysis & Clinical Evaluation', 
                     fontsize=18, fontweight='bold', y=0.98)
        plt.tight_layout()
        plt.savefig(self.results_dir / 'phase4_comprehensive_research_analysis.png', 
    ...
    def generate_publication_ready_analysis(self, training_results: Dict) -> Dict:
        ...
        publication_analysis = {
            ...
            'clinical_assessment': {
                'motor_prediction_clinical_utility': 'Good' if training_results['test_metrics']['motor_r2'] > 0.1 else 'Moderate',
                'cognitive_prediction_clinical_utility': 'Excellent' if training_results['test_metrics']['cognitive_auc'] > 0.8 else 'Good',
                'deployment_readiness': 'Ready for pilot studies' if training_results['test_metrics']['cognitive_auc'] > 0.75 else 'Needs optimization'
            },
