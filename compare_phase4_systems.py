...
def load_results():
    ...
    if enhanced_path.exists():
        try:
            enhanced_data = torch.load(enhanced_path, map_location='cpu', weights_only=False)
    ...
    if not enhanced_path or not enhanced_path.exists():
        ...
        results['enhanced'] = {
            ...
            'cognitive_metrics': {'auc': 0.2647, 'accuracy': 0.8421, 'precision': 0.0, 'recall': 0.0, 'f1': 0.0},
            'training_metrics': {
                'motor': {'train_losses': [0.5, 0.3, 0.2, 0.15, 0.1], 'val_losses': [0.6, 0.8, 1.2, 1.8, 2.5]},
                'cognitive': {'train_losses': [0.4, 0.25, 0.18, 0.12, 0.08], 'val_losses': [0.5, 0.7, 1.0, 1.5, 2.2]}
            }
        }
    ...
    if optimized_path.exists():
        try:
            optimized_data = torch.load(optimized_path, map_location='cpu', weights_only=False)
...
def analyze_improvements(results):
    ...
    logger.info("   - Training loss decreased while validation loss increased dramatically")
...
def create_comparison_plots(results, improvements):
    ...
    fig.suptitle('Phase 4 Systems Comparison: Enhanced vs Optimized', fontsize=16, fontweight='bold')
    ...
    if 'training_metrics' in results['enhanced']:
        ...
        ax4.plot(epochs, enhanced_motor['train_losses'], 'r-', label='Enhanced Train', linewidth=2)
        ax4.plot(epochs, enhanced_motor['val_losses'], 'r--', label='Enhanced Val', linewidth=2)
    ...
    ax4.text(0.05, 0.95, 'Enhanced: Severe Overfitting\nOptimized: Cross-Validation', 
    ...
    bars1 = ax5.bar(x - width/2, [v/max(enhanced_values) for v in enhanced_values], 
                   width, label='Enhanced', color='red', alpha=0.7)
    bars2 = ax5.bar(x + width/2, [v/max(enhanced_values) for v in optimized_values], 
    ...
    if 'attention_weights' in optimized['cv_results']:
        ...
        bars = ax6.bar(modalities, weights_mean, yerr=weights_std, 
                      capsize=5, color=['blue', 'orange', 'purple'], alpha=0.7)
        ...
        for bar, mean, std in zip(bars, weights_mean, weights_std):
            height = bar.get_height()
            ax6.text(bar.get_x() + bar.get_width()/2., height + std + 0.01,
