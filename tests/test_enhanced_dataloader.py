...
class TestPPMIDataLoader:
    ...
    def sample_config(self):
        ...
        return {
            ...
            'data_sources': {
                ...
                'xing_core_lab': {'filename': 'Xing_Core_Lab_-_Quant_SBR_18Sep2025.csv'}
            }
        }
    ...
    def test_assess_data_quality_excellent(self, temp_config_file, sample_dataframe):
        ...
        assert metrics.total_features == len(high_quality_df.columns) - 1  # Exclude PATNO
        assert metrics.completeness_rate >= 0.95
        assert metrics.quality_category == 'excellent'
    ...
    def test_assess_data_quality_categories(self, temp_config_file):
        ...
        for completeness_rate, expected_category in test_cases:
            ...
            for i, col in enumerate(data_cols):
                ...
                if col_missing > 0:
                    missing_indices = np.random.choice(n_rows, size=min(col_missing, n_rows), replace=False)
    ...
    def test_identify_dicom_patients(self, temp_config_file):
        ...
        data_dict = {
            'demographics': pd.DataFrame({
                'PATNO': [3001, 3002, 3003, 3004, 3005],
                'age': [65, 70, 55, 60, 75]
            }),
            ...
            'xing_core_lab': pd.DataFrame({
                'PATNO': [3002, 3003, 3004],  # Imaging data for 3 patients (overlap with FS7)
                'EVENT_ID': ['BL', 'BL', 'BL'],
    ...
    def test_load_with_quality_metrics(self, mock_load_csv, temp_config_file, sample_dataframe):
    ...
    def test_get_dicom_cohort_statistics(self, temp_config_file):
        ...
        # Create mock data
        data_dict = {
            'demographics': pd.DataFrame({'PATNO': range(3001, 3101)}),  # 100 total patients
            'fs7_aparc_cth': pd.DataFrame({'PATNO': range(3001, 3048)})   # 47 DICOM patients
        }
    ...
    def test_generate_quality_summary(self, temp_config_file, sample_dataframe):
        ...
        quality_reports = {
            'dataset1': DataQualityReport('dataset1', metrics1, True, [], datetime.now(), '/path1'),
            'dataset2': DataQualityReport('dataset2', metrics2, True, [], datetime.now(), '/path2')
        }
...
class TestIntegration:
    ...
    def integration_config(self):
        ...
        return {
            ...
            'quality_thresholds': {'excellent': 0.95, 'good': 0.80, 'fair': 0.60, 'poor': 0.40},
            ...
            'data_sources': {
                'test_dataset': {'filename': 'test_data.csv'}
            }
        }
    ...
    def test_end_to_end_workflow(self, integration_config):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as csv_file:
            ...
            integration_config['data_sources']['test_dataset']['filename'] = Path(csv_file.name).name
