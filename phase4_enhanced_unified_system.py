...
class EnhancedUnifiedAttentionModule(nn.Module):
    ...
    def forward(self, spatial_features, genomic_features, temporal_features):
        ...
        temporal_processed = self.feature_processor(temporal_features) + temporal_features
        
        # Stack for attention computation
        stacked_features = torch.stack([
            spatial_processed, genomic_processed, temporal_processed
        ...
        attention_scores = torch.einsum('bmd,md->bm', attended_features, self.attention_weights) / self.attention_temperature
        cross_modal_weights = F.softmax(attention_scores, dim=1)
        
        # Apply attention
        unified_features = torch.einsum('bm,bmd->bd', cross_modal_weights, attended_features)
...
class EnhancedUnifiedGIMANSystem(nn.Module):
    ...
    def forward(self, spatial_emb: torch.Tensor, genomic_emb: torch.Tensor, 
        ...
        unified_features, cross_modal_weights, self_attention_weights = self.unified_attention(
        ...
        if return_attention:
            return (motor_pred, cognitive_pred, unified_features, 
                   cross_modal_weights, self_attention_weights)
    ...
    def generate_counterfactuals(self, features: torch.Tensor, target_changes: torch.Tensor) -> torch.Tensor:
...
class EnhancedPhase4SystemTrainer:
    ...
    def prepare_data(self, spatial_embeddings: np.ndarray, genomic_embeddings: np.ndarray,
                    temporal_embeddings: np.ndarray, motor_scores: np.ndarray, 
                    cognitive_labels: np.ndarray) -> Tuple[DataLoader, DataLoader, DataLoader]:
    ...
    def train_model(self, train_loader, val_loader, num_epochs=100):
        ...
        logger.info(f"📋 Config: LR={self.config.learning_rate}, WD={self.config.weight_decay}, "
        ...
        for epoch in range(num_epochs):
            ...
            # Training phase
            self.model.train()
            train_metrics = self._train_epoch(train_loader, optimizer, mse_loss, bce_loss)
            ...
            if epoch % 5 == 0 or epoch < 10:
                logger.info(f"Epoch {epoch:3d}: Train={train_metrics['total_loss']:.4f}, "
                           f"Val={val_metrics['total_loss']:.4f}, LR={current_lr:.6f}")
                logger.info(f"           Motor: {train_metrics['motor_loss']:.4f}→{val_metrics['motor_loss']:.4f}, "
        ...
        logger.info(f"✅ Enhanced training completed. Best validation loss: {best_val_loss:.4f}")
    ...
    def _train_epoch(self, train_loader, optimizer, mse_loss, bce_loss):
        ...
        for batch_idx, (spatial, genomic, temporal, motor_scores, cognitive_labels) in enumerate(train_loader):
            ...
            combined_loss = motor_weight * motor_loss + cognitive_weight * cognitive_loss
    ...
    def evaluate_model(self, test_loader):
        ...
        with torch.no_grad():
            for spatial, genomic, temporal, motor_scores, cognitive_labels in test_loader:
                spatial = spatial.to(self.device)
                genomic = genomic.to(self.device)
                temporal = temporal.to(self.device)
...
def main():
    ...
    np.random.seed(42)
    temporal_embeddings = np.random.randn(len(integrator.patient_ids), config.embed_dim).astype(np.float32)
