#!/usr/bin/env python3
"""
Phase 2 Step 2.8v5 — Phase 1 vs Phase 2 Posterior Predictive Check (IS-posterior, SBR+CSF edition)
==========================================================================================
...
CHAINS_IS_V5 = POST_DIR / "chains_is_v5"
V5_SUMMARY = POST_DIR / "phase2_coupled_is_step26v5_csf.csv"
...
RNG_SEED = 202604102  # distinct from step_2_6_v4's seed
...
def main() -> int:
    print("=" * 76)
    print("Phase 2 Step 2.8v5 — PPC on IS posterior (Phase 1 vs Phase 2)")
    print("=" * 76)

    # --- Provenance header (closed-loop v1.0 rule) ---
    input_files = [V5_SUMMARY, LONG_PARQUET]
    ...
    sample_chains = sorted(CHAINS_IS_V5.glob("PATNO_*.parquet"))[:3]
    ...
    provenance = capture_provenance(
        ...
        extra={
            ...
            "chains_dir": str(CHAINS_IS_V5.relative_to(REPO_ROOT)),
        },
    )
    git = provenance["git"]
    ...
    if not V5_SUMMARY.exists():
        print(f"ERROR: {V5_SUMMARY} not found — run step_2_6_v4 first.")
        return 1

    summary_v4 = pd.read_csv(V5_SUMMARY)
    long_df = pd.read_parquet(LONG_PARQUET).sort_values(["PATNO", "t_years"]).reset_index(drop=True)
    chain_files = sorted(CHAINS_IS_V5.glob("PATNO_*.parquet"))
    ...
    # --- Persist ---
    csv_out  = OUT_DIR / "step_2_8_v5_ppc.csv"
    json_out = OUT_DIR / "step_2_8_v5_ppc_summary.json"
    md_out   = OUT_DIR / "step_2_8_v5_ppc_report.md"
    fig_out  = OUT_DIR / "step_2_8_v5_ppc.png"
    manifest_out = OUT_DIR / "step_2_8_v5_RUN_MANIFEST.md"
    ...
    fig.suptitle(f"Phase 2 Step 2.8v5 — PPC on IS posterior (N={n_pat})",
    ...
    summary = {
        ...
        "_output_hashes": {
            "step_2_8_v5_ppc.csv": _sha256(csv_out),
            "step_2_8_v5_ppc.png": _sha256(fig_out),
        },
    }
    json_out.write_text(json.dumps(summary, indent=2, default=str))
    summary["_output_hashes"]["step_2_8_v5_ppc_summary.json"] = _sha256(json_out)
    print(f"CSV:  {csv_out}")
    ...
    # --- Markdown report ---
    md_lines = [
        "# Phase 2 Step 2.8v5 — Posterior Predictive Check (IS-posterior)",
        "",
        ...
        f"**Run manifest:** [step_2_8_v5_RUN_MANIFEST.md](step_2_8_v5_RUN_MANIFEST.md)",
        "",
    ...
    write_run_manifest(
        ...
        step_name="Phase 2 Step 2.8v5 — PPC on IS posterior",
