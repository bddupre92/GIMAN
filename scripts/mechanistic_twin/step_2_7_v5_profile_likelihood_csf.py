#!/usr/bin/env python3
"""
Phase 2 Step 2.7v5 — Profile-Likelihood Practical Identifiability Diagnostic (IS-posterior SBR+CSF edition)
=====================================================================================================
...
weighted posteriors from Step 2.6v5 and tests the same T_tox stiff-direction
...
identifiable per Villaverde 2016. Step 2.6v5 persists the full joint
...
Inputs
------
outputs/mechanistic_twin/data/posteriors/phase2_coupled_is_step26v5_csf.csv
  -- per-patient IS summary with ESS fraction + correlations
outputs/mechanistic_twin/data/posteriors/chains_is_v5/PATNO_*.parquet
  -- per-patient IS-resampled joint chains (k_n, alpha_tox, T_tox, N=5000 each)

Outputs
-------
outputs/mechanistic_twin/phase2/step_2_7_v5_profile_likelihood_is.csv
outputs/mechanistic_twin/phase2/step_2_7_v5_profile_likelihood_is_summary.json
outputs/mechanistic_twin/phase2/step_2_7_v5_profile_likelihood_is_report.md
outputs/mechanistic_twin/phase2/step_2_7_v5_RUN_MANIFEST.md
outputs/mechanistic_twin/phase2/step_2_7_v5_profile_likelihood_is.png
...
See step_2_6_v5 script header for DOIs.
"""
...
CHAINS_IS    = POST_DIR / "chains_is_v5"
V4_SUMMARY   = POST_DIR / "phase2_coupled_is_step26v5_csf.csv"
...
def main() -> int:
    print("=" * 76)
    print("Phase 2 Step 2.7v5 — Profile-Likelihood Diagnostic (IS-posterior, SBR+CSF)")
    print("=" * 76)

    # --- Provenance header (closed-loop v1.0 rule) ---
    # Sample 3 chain files to hash alongside the summary CSV, as a cheap
    # sentinel that the full chains_is_v5/ tree is consistent.
    ...
    if not V4_SUMMARY.exists():
        print(f"ERROR: v5 summary CSV not found: {V4_SUMMARY}")
        print("       Run step_2_6_v5_is_weighted_posterior.py first.")
        return 1

    summary_v4 = pd.read_csv(V4_SUMMARY)
    chain_files = sorted(CHAINS_IS.glob("PATNO_*.parquet"))
    print(f"v5 summary rows: {len(summary_v4)}")
    ...
    # --- Persist outputs ---
    csv_out  = OUT_DIR / "step_2_7_v5_profile_likelihood_is.csv"
    json_out = OUT_DIR / "step_2_7_v5_profile_likelihood_is_summary.json"
    md_out   = OUT_DIR / "step_2_7_v5_profile_likelihood_is_report.md"
    fig_out  = OUT_DIR / "step_2_7_v5_profile_likelihood_is.png"
    manifest_out = OUT_DIR / "step_2_7_v5_RUN_MANIFEST.md"
    ...
    fig.suptitle(f"Phase 2 Step 2.7v5 — Profile-Likelihood on IS posterior (N={n_total})",
    ...
    output_hashes = {
        "step_2_7_v5_profile_likelihood_is.csv": _sha256(csv_out),
        "step_2_7_v5_profile_likelihood_is.png": _sha256(fig_out),
    }
    summary = {
    ...
    output_hashes["step_2_7_v5_profile_likelihood_is_summary.json"] = _sha256(json_out)
    print(f"JSON: {json_out}")

    # --- Markdown report ---
    md_lines = [
        "# Phase 2 Step 2.7v5 — Profile-Likelihood Diagnostic (IS-posterior, SBR+CSF)",
        "",
        f"**Date:** {provenance['datetime_utc']}",
        f"**Input:** {len(chain_files)} chains from `chains_is_v5/` + `phase2_coupled_is_step26v5_csf.csv`",
        f"**Git:** `{git.get('sha','?')}`{' DIRTY' if git.get('dirty') else ''}",
        f"**Run manifest:** [step_2_7_v5_RUN_MANIFEST.md](step_2_7_v5_RUN_MANIFEST.md)",
        "",
    ...
    write_run_manifest(
        ...
        step_name="Phase 2 Step 2.7v5 — Profile-Likelihood (IS-posterior, SBR+CSF)",
