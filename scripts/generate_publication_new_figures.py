...
def _figure12_counterfactual(output_path: Path) -> None:
    # Figure 12 is now generated by the direct counterfactual analysis script:
    #   archive/development/phase8/subphase8_2_dynamic_endpoints/run_counterfactual_analysis.py
    # That script produces the two-panel figure and saves it directly.
    # Here we just copy the latest generated figure.
    src = ROOT / "visualizations/publication_New/Figure12_Counterfactual_Trajectory.png"
    if src.exists() and src.resolve() != output_path.resolve():
        _copy(src, output_path)
    elif not src.exists():
        # Fallback: run the counterfactual analysis script
