...
class EnhancedDatasetCreator:
    ...
    def __init__(self, data_dir: str = "data", output_dir: str = "data/enhanced"):
        ...
        print("🚀 Enhanced Dataset Creator v1.1.0 Initialized")
    ...
    def load_production_graph_data(self) -> Data:
        ...
        graph_path = Path(
            "models/registry/giman_binary_classifier_v1.0.0/graph_data.pth"
        )
    ...
    def extract_enhancement_features(self, dataset: pd.DataFrame) -> pd.DataFrame:
        ...
        for feature in self.feature_mapper.enhancement_features:
            if feature in dataset.columns:
                ...
                print(
                    f"   ✅ {feature}: {coverage:.1f}% coverage, {extraction_results[feature]['unique_values']} unique values"
                )
            else:
    ...
    def create_enhanced_feature_matrix(self) -> tuple[pd.DataFrame, dict]:
        ...
        enhancement_features_df, enhancement_mapping = (
            self.extract_enhancement_features(source_dataset)
        )

        # Merge current and enhancement features
        if (
            "PATNO" in current_features_df.columns
            and "PATNO" in enhancement_features_df.columns
        ):
            enhanced_df = pd.merge(
                current_features_df, enhancement_features_df, on="PATNO", how="inner"
            )
        else:
            # If no PATNO, assume same order (risky but fallback)
            enhanced_df = pd.concat(
                [current_features_df, enhancement_features_df], axis=1
            )
        ...
        if "PATNO" in enhanced_df.columns:
            ...
            print(
                f"📊 Longitudinal data: {unique_patients} patients, {total_visits} visits (avg: {avg_visits:.1f} visits/patient)"
            )
        else:
    ...
    def impute_missing_values(self, dataset: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
        ...
        continuous_features = [
            f
            for f in self.feature_mapper.all_features
            if f in dataset.columns
            and self.feature_mapper.feature_types[f] == "continuous"
        ]
        ...
        categorical_features = [
            ...
            for f in self.feature_mapper.all_features
            if f in dataset.columns
            and self.feature_mapper.feature_types[f] in ["binary", "ordinal"]
        ]

        # Impute continuous features with KNN (better for biomarkers)
        if continuous_features:
            print(
                f"   🔄 Imputing {len(continuous_features)} continuous features with KNN..."
            )
            ...
            if missing_before > 0:
                ...
                imputation_stats["continuous"] = {
                    ...
                    "missing_after": int(
                        imputed_df[continuous_features].isnull().sum().sum()
                    ),
        ...
        if categorical_features:
            print(
                f"   🔄 Imputing {len(categorical_features)} categorical features with mode..."
            )
            ...
            if missing_before > 0:
                ...
                imputation_stats["categorical"] = {
                    ...
                    "missing_before": int(missing_before),
                    "missing_after": int(
                        imputed_df[categorical_features].isnull().sum().sum()
                    ),
        ...
        final_missing = (
            imputed_df[self.feature_mapper.all_features].isnull().sum().sum()
        )
        if final_missing > 0:
            print(
                f"   ⚠️ Warning: {final_missing} missing values remain after imputation"
            )
        else:
    ...
    def create_enhanced_graph_data(self, dataset: pd.DataFrame) -> Data:
        ...
        print("✅ Enhanced graph data created:")
        print(f"   📊 Nodes: {enhanced_graph_data.num_nodes}")
        print(f"   📊 Edges: {enhanced_graph_data.num_edges}")
        print(
            f"   📊 Features: {enhanced_graph_data.x.shape[1]} (enhanced from 7 to 12)"
        )
...
def main():
    ...
    try:
        ...
        print(
            f"📊 Features: 7 → 12 ({len(creator.feature_mapper.enhancement_features)} added)"
        )
        ...
        print(
            f"Added Features: {', '.join(creator.feature_mapper.enhancement_features)}"
        )
