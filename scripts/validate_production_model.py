...
def validate_model_integrity(model_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        print(
            f"   ✅ Graph: {graph_data.x.shape[0]} nodes, {graph_data.x.shape[1]} features"
        )
        ...
        checkpoint = torch.load(
            model_dir / "final_binary_giman.pth", map_location="cpu", weights_only=False
        )
        ...
        if param_count != expected_params:
            warning_msg = (
                f"Parameter count differs: {param_count} (expected {expected_params})"
            )
        ...
        with torch.no_grad():
            ...
            print(
                f"   ✅ Predictions: {class_0_count} healthy, {class_1_count} diseased"
            )
            ...
            results["metrics"] = {
                ...
                if logits.shape[1] == 1
                else probs[:, 1].mean().item(),
                "probability_std": probs.std().item()
                if logits.shape[1] == 1
                else probs[:, 1].std().item(),
            }
        ...
        if hasattr(graph_data, "y") and graph_data.y is not None:
            ...
            if abs(auc_roc - expected_auc) > 0.01:  # Allow 1% tolerance
                ...
                print("   ✅ Performance matches expected values")
...
def compare_model_outputs(model_path1: str, model_path2: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"comparison_successful": False, "error": str(e)}
...
def main():
    ...
    if args.compare:
        ...
        if comparison.get("comparison_successful"):
            ...
            print(
                f"   Max difference: {comparison.get('max_output_difference', 'N/A'):.2e}"
            )
        else:
            print(
