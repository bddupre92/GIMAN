...
def load_trained_model():
    ...
    if not model_dirs:
        print("   📋 Available directories in models/:")
...
def run_comprehensive_analysis():
    ...
    explainer.visualize_node_importance(
        node_importance, save_path=vis_dir / "node_importance_analysis.png"
    )
    ...
    for i, feature in enumerate(feature_importance["ranked_features"]):
        importance = feature_importance["feature_importance"][
            feature_importance["importance_ranking"][i]
        ]
        print(f"   {i + 1:2d}. {feature:<20}: {importance:.6f}")

    # Visualize feature importance
    explainer.visualize_feature_importance(
        feature_importance, save_path=vis_dir / "feature_importance_analysis.png"
    )
    ...
    for i, edge in enumerate(edge_contributions["edge_contributions"][:10]):
        print(
            f"   {i + 1:2d}. Edge {edge['source']} -> {edge['target']}: "
    ...
    print(
        f"   • Importance concentration (CV): {insights['importance_concentration']:.3f}"
    )
    ...
    print("\n📊 GRAPH STATISTICS:")
    print(
        f"   • Average degree: {graph_stats['degree_stats']['mean']:.2f} ± {graph_stats['degree_stats']['std']:.2f}"
    )
    print(
        f"   • Degree range: {graph_stats['degree_stats']['min']} - {graph_stats['degree_stats']['max']}"
    )
    ...
    node_analysis = explainer.compare_predictions_with_without_edges(
        target_node=most_important_node, num_edges_to_remove=5
    )
    ...
    print("   • Top edge impacts:")

    for i, edge in enumerate(node_analysis["edge_removal_analysis"][:3]):
        print(
            f"     {i + 1}. Edge to node #{edge['target_node_in_edge']}: "
    ...
    print("   • Node importance plots: node_importance_analysis.png")
    print("   • Feature importance plots: feature_importance_analysis.png")
    print("   • Comprehensive report: giman_interpretation_report.json")
...
def validate_model_authenticity():
    ...
    with torch.no_grad():
        ...
        print(
            f"   • Prediction difference with different architecture: {arch_diff:.8f}"
        )
